...
window.Utils = {
  ...
  setCookie: function (key, value, expireSeconds) {
    if (key && value) {
      ...
      expireDate.setTime(
        expireDate.getTime() + (expireSeconds || 60 * 60 * 24 * 30) * 1000
      );
  ...
  deleteCookie: function (key) {
    key &&
      (this.document.cookie = key + "=;expires=Thu, 01 Jan 1970 00:00:01 GMT;");
  },
  /**
  ...
  formatUrl: function (url) {
    ...
    return (
      url.substring(0, index) + url.substring(index).replace(/\/\/*/g, "/")
    );
  },
  isIE: function () {
